<!DOCTYPE HTML>
<html>
	<head>
		<title>phrits.com &ndash; Recipes</title>

<?php include '/head.html'; ?>

	</head>

	<body class="is-preload">
		<!-- Header -->
		<header id="header">
			<a href="index.html" class="title phrits-invert">phrits.com</a>
			<nav>
				<ul>
					<li><a href="/">Welcome</a></li>
					<li><a href="#itProfessional">IT Professional</a></li>
					<li><a href="#foodGeek" class="active">Food Geek</a></li>
					<li><a href="#miscellany">Miscellany</a></li>
				</ul>
			</nav>
		</header>

		<!-- Wrapper -->
		<div id="wrapper">
				<!-- Main -->
			<section id="main" class="wrapper">
				<h1 class="major">Recipes</h1>
				<span class="image fit"><img src="images/board_and_knife.jpg" alt="" /></span>

				<!-- Charleston Grits-->
				<div class="wrapper style3 recipe">
					<a href="/recipes/charleston_grits/" class="image right">
						<img src="/images/edisto_beach.jpg" alt="Edisto Beach, SC" title="Edisto Beach, SC" />
					</a>
					<h2><a href="/recipes/grits/">Charleston Grits</a></h2>
					<p>
						A <a href="http://en.wikipedia.org/wiki/South_Carolina_Lowcountry">South Carolina Lowcountry</a> classic I first tried at the always wonderful <a href="http://www.theoldpostofficerestaurant.com/">Old Post Office Restaurant</a> in <a href="http://www.edistobeach.com/">Edisto Beach, South Carolina</a>. [
							<span class="dietary">
								Vegetarian
							</span>
						]
					</p>
				</div> <!-- class="wrapper style3 recipe" -->
			</section>
	</div> <!-- wrapper -->

<?php include '/footer.html'; ?>

<?php include '/scripts.html'; ?>

	</body>
</html>